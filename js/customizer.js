...
( function ( exports, $ ) {
	...
	api.myCustomizerPreviewer = {
		...
		init: function () {
			...
			if (! $("#customize-preview iframe").hasClass('themes_multisite_active') ) {

					$('#customize-preview iframe').contents().find('.themes-partial.customize-partial-edit-shortcut').each( function(){
						$(this).removeClass('active');
					} );
				}
			} );

			// activated themes partial icons
			...
			$('#customize-controls h3.themes-group-heading').each(function(){
				if($(this).next('.themes-group-info').length>0){
					$(this).next('.themes-group-info').hide();
			...
			$(document).on('click', '#customize-controls h3.themes-group-heading .customize-help-toggle', function(){
				$(this).parent().next('.themes-group-info').slideToggle();
			});
