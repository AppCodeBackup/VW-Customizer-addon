...
if ( ! class_exists( 'ThemeSetting' ) ) :
  ...
  final class ThemeSetting {
    ...
    */
    private function define_constants() {
      ...
      $this->define( 'CUSTOM_FEEDBACK_SERVER', 'https://wpbrigade.com/' );
      }
 ...
 function _send_data( $args ) {
  ...
  /**
	...
	public function themes_action_links( $links, $file ) {
      ...
      if ( ! has_action( 'themes_pro_add_template' ) ) :
        $pro_link = sprintf( esc_html__( '%1$s %3$s Upgrade Pro %4$s %2$s', 'themes' ),  '<a href="https://wpbrigade.com/wordpress/plugins/themes-pro/?utm_source=themes-lite&utm_medium=plugin-action-link&utm_campaign=pro-upgrade" target="_blank">', '</a>', '<span class="themes-dasboard-pro-link">', '</span>' );
  ...
  //     $raw_response = wp_remote_post( 'https://wpbrigade.com/themes-api/search.php', array(
  //       'body' => array(
  //         'slug' => $args->slug
  //       ) )
  //      );
...
if (!class_exists('TAV_Remote_Notification_Client')) {
  require( CUSTOM_ROOT_PATH . 'include/class-remote-notification-client.php' );
}
$notification = new TAV_Remote_Notification_Client( 125, '16765c0902705d62', 'https://wpbrigade.com?post_type=notification' );

register_activation_hook( __FILE__, array( 'ThemeSetting', 'plugin_activation' ) );
register_uninstall_hook( __FILE__, array( 'ThemeSetting', 'plugin_uninstallation' ) );
