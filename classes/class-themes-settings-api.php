...
/**
 * ThemeSetting Settings API
 *
 * @since 1.0.9
 */
if ( !class_exists( 'Themes_Setting_Settings_API' ) ):

  class Themes_Setting_Settings_API {
    ...
    */
    function admin_enqueue_scripts( $hook ) {
      ...
      if( $hook != 'toplevel_page_themes-settings' ) {
        return;
      }
    ...
    */
    function callback_checkbox( $args ) {
      ...
      $html  .= sprintf( '<input type="checkbox" class="checkbox themes-check-hidden" id="wpb-%1$s[%2$s]" name="%1$s[%2$s]" value="on" %3$s />', $args['section'], $args['id'], checked( $value, 'on', false ) );
      $html  .= sprintf( '%2$s%1$s</label>', $args['desc'], '<span class="themes-checkbox"></span>' );
      $html  .= '</fieldset>';
    ...
    */
    function callback_multicheck( $args ) {
      ...
      foreach ( $args['options'] as $key => $label ) {
        ...
        $html    .= sprintf( '<input type="checkbox" class="checkbox themes-check-hidden" id="wpb-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" value="%3$s" %4$s />', $args['section'], $args['id'], $key, checked( $checked, $key, false ) );
        $html    .= sprintf( '%2$s%1$s</label><br>',  $label, '<span class="themes-checkbox"></span>' );
      }
    ...
    */
    function callback_autologin( $args ) {
      ...
      $html = apply_filters( 'themes_autologin', $args );
      $html .= $this->get_field_description( $args );
    ...
    */
    function callback_hidelogin( $args ) {
      ...
      $value = esc_attr( $this->get_option( $args['id'], $args['section'], $args['std'] ) );
      $html  = apply_filters( 'themes_hidelogin', $args, $value );
      $html .= $this->get_field_description( $args );
    ...
    */
    function callback_login_redirect( $args ) {
      ...
      $html = apply_filters( 'themes_login_redirects', $args );
      $html  .= $this->get_field_description( $args );
    ...
    */
    function callback_register_fields( $args ) {
      ...
      $html = apply_filters( 'themes_register_fields', $args );
      $html  .= $this->get_field_description( $args );
    ...
    */
    function show_forms() {
      ?>
      <div class="metabox-holder themes-settings">
        <?php foreach ( $this->settings_sections as $form ) : ?>
          <div id="<?php echo $form['id']; ?>" class="group" style="display: none;">
            <form method="post" action="options.php">
              <?php
              $remove_submit = array( 'themes_autologin', 'themes_login_redirects', 'themes_register_fields' );
            ...
            <?php
               ...
               * @return string
               */
              if ( $form['id'] == 'themes_autologin' ) :
                do_action( 'themes_autologin_script' );
              endif;
              /**
               ...
               * @return string
               */
              if ( $form['id'] == 'themes_login_redirects' ) :
                do_action( 'themes_login_redirect_script' );
              endif;
              /**
               ...
               * @return string
               */
              if ( $form['id'] == 'themes_limit_login_attempts' ) :
                do_action( 'themes_limit_login_attempts_log_script' );
                do_action( 'themes_limit_login_attempts_whitelist_script' );
                do_action( 'themes_limit_login_attempts_blacklist_script' );
              endif;
              /**
               ...
               * @return string
               */
              if ( $form['id'] == 'themes_register_fields' ) :
                do_action( 'themes_register_fields_script' );
              endif;
              /**
               ...
               * @return string
               */
              if ( $form['id'] == 'themes_social_logins' ) :
                do_action( 'themes_social_login_help_tab_script' );
              endif;
              ?>
          </div>
        <?php endforeach; ?>
    ...
    function script() {
      ...
      jQuery(document).ready(function($) {
        ...
        $('.group .collapsed').each(function(){
          ...
          $('.nav-tab-wrapper a:not(".themes-premium")').click(function(evt) {
